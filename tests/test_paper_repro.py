...
def test_ensure_nltk_data_skips_download_when_already_present(monkeypatch):
    """nltk.download() always hits the network to refresh its package
    index first (Downloader._update_index has no on-disk cache), so a
    resource already on disk must short-circuit via nltk.data.find()
    instead of calling download() at all -- otherwise every run pays a
    network round trip (and is exposed to the transient ParseError below)
    even when nothing needs fetching.

    Monkeypatches the real `nltk` module object (the one `import nltk`
    inside ensure_nltk_data() resolves to via sys.modules), not a copy --
    module objects are singletons, so patching attributes here is visible
    to the function's own local import.
    """
    import nltk

    def _fail_if_called(resource, quiet=True):
        raise AssertionError(f"download() should not be called for cached resource {resource!r}")

    monkeypatch.setattr(nltk, "download", _fail_if_called)

    ensure_nltk_data()  # all three resources are already on disk from the test above


def test_ensure_nltk_data_retries_transient_index_parse_error(monkeypatch):
    """Real bug hit on a live run: nltk.download() raised a bare
    xml.etree.ElementTree.ParseError("unclosed token") from deep inside
    Downloader._update_index -- a truncated fetch of nltk's package-index
    XML over an unreliable connection, not anything in this project or
    the vendored submodule. Confirms ensure_nltk_data() retries instead
    of propagating the first transient failure."""
    import time
    import xml.etree.ElementTree as ET

    import nltk

    from amem_gepa.paper_repro import _NLTK_RESOURCE_PATHS, ensure_nltk_data

    monkeypatch.setattr(nltk.data, "find", lambda path: (_ for _ in ()).throw(LookupError()))
    monkeypatch.setattr(time, "sleep", lambda seconds: None)

    attempts = []

    def _flaky_download(resource, quiet=True):
        attempts.append(resource)
        if len(attempts) % 3 != 0:
            raise ET.ParseError("unclosed token: line 65, column 4")

    monkeypatch.setattr(nltk, "download", _flaky_download)

    ensure_nltk_data()

    assert len(attempts) == 3 * len(_NLTK_RESOURCE_PATHS)


def test_ensure_nltk_data_raises_clear_error_after_exhausting_retries(monkeypatch):
    import time
    import xml.etree.ElementTree as ET
    ...
    def _always_fails(resource, quiet=True):
        raise ET.ParseError("unclosed token: line 65, column 4")

    monkeypatch.setattr(nltk, "download", _always_fails)
    monkeypatch.setattr(time, "sleep", lambda seconds: None)

    with pytest.raises(RuntimeError, match="Failed to download NLTK resource"):
        ensure_nltk_data()
